...
class Plugin {
  ...
  addDepartmentPlugin = (req, res) => {
    console.log(req);
    const url = "http://localhost:4000/department";
    axios({
      method: "post",
      url: url,
      data: req.body,
      ...
      .catch((err) => {
        console.log(err);
        return res.status(501).send("internal department failed add");
      });
  };
  ...
  getDepartmentByIdPlugin = async (req, res) => {
    const url = "http://localhost:4000/department/" + req.params.id;
    const response = await fetch(url);
    const jsonData = await response.json();
    console.log(jsonData);
    return res.status(200).send(jsonData);
